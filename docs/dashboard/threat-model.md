...
| Filtros e exportação de histórico | Tampering | **Injeção em filtros / CSV formula injection** ao exportar linha começando com `=`, `+`, `-` ou `@` (executada por Excel/Sheets) | Filtros são tipados no estado React (selects controlados). `escapeCsvValue()` **prefixa com aspa simples** qualquer valor que comece com `= + - @` (ou tab/CR), forçando o conteúdo a texto, além de escapar aspas/delimitadores. | Integridade / segurança de exportação | `dashboard/src/utils/csv.ts`; **teste automatizado** em `dashboard/src/__tests__/csv.test.ts`. |
...
- Evidência anexada: `docs/dashboard/evidencias/npm-audit-a1-8.txt`
...
4. **`baseUrl` na mensagem de erro do `HttpDashboardClient`**: a URL base (configuração de ambiente, não segredo nesta release) aparece na mensagem de erro lançada. Risco baixo; remoção planejada no Marco 4 ao ligar a API real e exigir HTTPS. (Levantado na peer-audit, achado F10.)
